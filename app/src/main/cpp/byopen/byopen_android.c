    ...
    if (fp) {
        while (fgets(line, sizeof(line), fp)) {
            if (strstr(line, filename)) {
                ...
                if (3 == sscanf(line, "%"SCNxPTR
                                      "-%*"
                                      SCNxPTR
                                      " %4s %"
                                      SCNxPTR
                                      " %*x:%*x %*d%n", &start, page_attr, &offset, &pos)) {
    ...
    if (symtab && strtab) {
        for (i = 0; i < symtab_num; i++, symtab++) {
            ...
            if ((by_pointer_t) name < end && strcmp(name, symbol) == 0 &&
                ...
                by_pointer_t symboladdr = (by_pointer_t) (dlctx->baseaddr + symtab->st_value -
                                                          dlctx->biasaddr);
                by_trace("dlsym(%s): found at .symtab/%p = %p + %x - %p", symbol, symboladdr,
                         dlctx->baseaddr, (by_int_t) symtab->st_value, dlctx->biasaddr);
                return symboladdr;
            }
        }
    }
